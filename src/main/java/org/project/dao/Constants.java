...
public interface Constants {
    ...
    //Order DAO
    String GET_ALL_CHECKOUTS = "SELECT c.id, start_time, end_time, is_returned, order_status, c.type, u.id, u.login, b.id, b.isbn FROM checkout c JOIN user u JOIN book b ON c.user_id = u.id AND c.book_id = b.id";
    String GET_CHECKOUTS_BY_LOGIN = "SELECT c.id, start_time, end_time, is_returned, order_status, c.type, u.id, u.login, b.id, b.isbn FROM checkout c JOIN user u JOIN book b ON c.user_id = u.id AND c.book_id = b.id WHERE order_status = 1 AND u.login = ?";
    String CREATE_CHECKOUT = "INSERT INTO checkout (start_time, end_time, is_returned, order_status, `type`, user_id, book_id) VALUES (?, ?, 1, 0, ?, ?, ?)";
